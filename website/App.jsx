...
async function fetchPosterFromTMDB(title, year) {
  ...
  // FIX: Pehle year ke saath try karo, agar result nahi mila to bina year ke try karo
  const trySearch = async (withYear) => {
    const searchUrl = `https://api.themoviedb.org/3/search/multi?api_key=${TMDB_API_KEY}&query=${encodeURIComponent(title)}${withYear && year ? `&year=${year}` : ""}&language=en-US&page=1`;
    const controller = new AbortController();
    ...
    const data = await res.json();
    return (data.results || []).find(r => r.poster_path) || null;
  };

  try {
    // Year ke saath pehle try karo
    let result = year ? await trySearch(true) : null;
    // Agar nahi mila to bina year ke try karo
    if (!result) result = await trySearch(false);
    if (!result) {
      // Agar ab bhi nahi mila, title ke pehle 2-3 words se try karo
      const shortTitle = title.split(" ").slice(0, 3).join(" ");
      if (shortTitle !== title) {
        const searchUrl = `https://api.themoviedb.org/3/search/multi?api_key=${TMDB_API_KEY}&query=${encodeURIComponent(shortTitle)}&language=en-US&page=1`;
        const controller = new AbortController();
        const timer = setTimeout(() => controller.abort(), 8000);
        const res = await fetch(searchUrl, { signal: controller.signal });
        clearTimeout(timer);
        if (res.ok) {
          const data = await res.json();
          result = (data.results || []).find(r => r.poster_path) || null;
        }
      }
    }
    ...
    // FIX: TMDB se actual release year nikalo (not from filename)
    const releaseDate = result.release_date || result.first_air_date || null;
    const tmdbYear = releaseDate ? releaseDate.substring(0, 4) : null;

    const out = { poster: posterUrl, imdb_rating: rating, plot, tmdbYear };
    posterCache[key] = out;
    return out;
  } catch {
    posterCache[key] = null;
...
function MovieCard({ file, onClick }) {
  ...
  // FIX: TMDB se actual year fetch karo
  useEffect(() => {
    fetchPosterFromTMDB(name, year).then(data => {
      if (data?.tmdbYear) setTmdbYear(data.tmdbYear);
    });
  }, [name, year]);

  const displayYear = tmdbYear || year;
  ...
  return (
    <div onClick={onClick} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      ...
      <div style={{ padding: "8px 8px 10px" }}>
        ...
        {displayYear && <span style={{ fontSize: 9, color: "#555" }}>{displayYear}</span>}
      </div>
    </div>
  );
}
    ...
    <div onClick={() => onClick(file)} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 5, marginBottom: 8 }}>
          ...
          {displayYear && <span style={{ padding: "2px 8px", borderRadius: 6, fontSize: 10, background: "#222", color: "#777" }}>{displayYear}</span>}
        </div>
...
function HeroBanner({ file, onClick }) {
  ...
  // FIX: TMDB year priority
  const displayYear = posterData?.tmdbYear || year;
    ...
    <div onClick={() => onClick(file)}
      ...
      <div style={{ position: "absolute", bottom: 0, left: 0, right: "25%", padding: "18px 20px" }}>
        ...
        <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
          ...
          {displayYear && <span style={{ fontSize: 11, color: "#777" }}>{displayYear}</span>}
        </div>
        {posterData?.plot && (
...
function DetailModal({ file, onClose }) {
  ...
  // FIX: TMDB year ko priority do, fallback filename year
  const displayYear = posterData?.tmdbYear || year;
      ...
      <div onClick={e => e.stopPropagation()} style={{ background: "#111", borderRadius: "24px 24px 0 0", width: "100%", maxWidth: 480, overflow: "hidden", animation: "slideUp .3s cubic-bezier(.32,1.4,.6,1)", maxHeight: "92vh", overflowY: "auto" }}>
        ...
        <div style={{ padding: "0 18px 34px" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: 7, marginBottom: 14 }}>
            ...
            {displayYear && <span style={{ padding: "4px 12px", borderRadius: 8, fontSize: 11, background: "#222", color: "#888" }}>{displayYear}</span>}
