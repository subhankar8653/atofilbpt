    ...
    try:
        async with aiohttp.ClientSession() as session:
            ...
            if poster == 'N/A' or not poster:
                ...
                # Upgrade poster quality — use high-res URL directly, no conversion
                import re as _re
                    ...
                    await bot.send_cached_media(
                        ...
                        reply_markup=InlineKeyboardMarkup(
                            [
                                ...
                                InlineKeyboardButton("Bᴏᴛ Oᴡɴᴇʀ", url="t.me/suhanibots")
                                ]
                            ]
                        )
                    )
...
async def get_cap(settings, remaining_seconds, files, query, total_results, search, is_filtered=False):
    if settings["imdb"]:
        ...
        for file in files:
            cap += f"<b><a href='https://telegram.me/{temp.U_NAME}?start=files_{file.file_id}'>📁 {get_size(file.file_size)} ▷ {' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.'), file.file_name.split()))}\n\n</a></b>"
    # Telegram 4096 char limit — file links ko safely trim karo
    if len(cap) > 4050:
        cap = cap[:4050].rsplit("\n", 1)[0] + "\n\n<b>…(aur files hain, filter use karo)</b>"
    return cap
